...
pthread_mutex_t *S;     //mutex for threads
...
void creatPhilosophers(int nthreads){
    ...
    S = (pthread_mutex_t *) malloc(nthreads * sizeof(pthread_mutex_t));    
...
void pickUpChopsticks(int threadIndex){
    ...
    while(state[threadIndex] == HUNGRY){    
        ...
        pthread_mutex_lock (&S[threadIndex]);
    }
    pthread_mutex_unlock (&mutex);
}
/*****************************************************************
...
void test(int threadIndex){
    if (state[threadIndex] == HUNGRY
        ...
        printf("Philosopher %d pick up chopstick %d and %d\n",
                    threadIndex + 1, LEFT + 1, threadIndex + 1);
        eating(threadIndex);
        pthread_mutex_unlock (&S[threadIndex]);                           
    }
}
